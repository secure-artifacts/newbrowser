...
| 范畴 | 1.1.11 改进 |
|---|---|
| Chromium 账号书签 | 新增 `AccountBookmarks` 扫描。登录账号书签与传统 `Bookmarks` 分开落盘时不再漏检。 |
| 备份残留 | 新增 `AccountBookmarks.bak`，并将当前书签与备份残留使用不同记录类型显示。 |
| 稳定读取 | 浏览器运行中先预留 3 秒落盘窗口；读取前后核对文件标识、大小与修改时间，Chrome 原子替换文件或短暂产生不完整 JSON 时最多重试三次。 |
| 结构兼容 | 不再只遍历 `roots`，而是遍历完整书签 JSON 中所有真实 `url` 节点，兼容账号书签和后续根结构变化。 |
| 加密书签 | 识别 `EncryptedBookmarks2`、`EncryptedAccountBookmarks2` 及旧文件名；仅存在加密文件时明确标记部分完成，禁止静默宣称无留痕。 |
| 诊断 | 对 `AccountBookmarks`、备份和命中文件记录不含网址的读取数量与命中数量，便于准确定位 Profile 差异。 |

### 根因

Chromium 官方将本地/传统书签保存到 `Bookmarks`，将登录账号书签保存到独立的 `AccountBookmarks`。v1.1.10 只读取前者，导致账号存储 Profile 漏检，而使用本地存储的另一个 Profile 可以正常命中。

### 验证重点

新增回归覆盖仅含 `AccountBookmarks` 的 Profile、同机本地与账号两种存储、非 `roots` 顶层结构、原子替换重试及仅加密书签文件状态。
