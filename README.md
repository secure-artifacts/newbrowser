...
当前发布目标为 **v1.1.9**。本版本在保留防卡死保护的同时，修复无 History Profile 漏识别，以及 History 异常导致书签被一并跳过的问题。完整变更见 [CHANGELOG.md](CHANGELOG.md)。
...
> 诊断文本可用于支持排障，但不应包含浏览器数据库、Cookie、登录数据、书签、真实网址或账号截图。复制诊断会移除 Profile 显示名中的账号信息，并将底层异常压缩为类型/错误代码。
...
```bash
# 1. 在 release/v1.1.9 分支完成审查、测试并合并到 main
...
如发现 1.1.9 重大问题，应停止分发对应资产、记录受影响范围，并在 `main` 基于已验证的提交发布递增修复版本。不要删除用户已下载的文件，也不要强制删除历史 Release 或覆盖既有 tag，以保留审计追溯能力。
